...
export function NewsBoard({ type }: { type?: "EVENT" | "NEWS" }) {
  ...
  const handlePageChange = (page: number) => {
    ...
    sendGAEvent({
      event: "news_pagination_changed",
      params: { previous_page: currentPage, new_page: page },
    })
  }

  const handleDelete = async (id: number) => {
    sendGAEvent({
      event: "news_item_delete_attempt",
      params: { post_id: id },
    })
    try {
      await deletePost(id)
      sendGAEvent({
        event: "news_item_delete_success",
        params: { post_id: id },
      })
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 min-w-0 pr-2">
                  ...
                  <Link
                    ...
                    onClick={() =>
                      sendGAEvent({
                        event: "news_item_clicked",
                        params: { post_id: post.id, type: post.type, is_pinned: post.isPinned },
                      })
                    }
                  >
